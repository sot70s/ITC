...
namespace ITC
{
    ...
    {
        public string GenerateIdentityV3Hash(string password, KeyDerivationPrf prf = KeyDerivationPrf.HMACSHA1, int iterationCount = 10000, int saltSize = 16)
        {
        ...
        private static byte[] ComposeIdentityV3Hash(byte[] salt, uint iterationCount, byte[] passwordHash)
        {
            ...
            Buffer.BlockCopy(ConvertToNetworkOrder((uint)KeyDerivationPrf.HMACSHA1), 0, hash, 1, sizeof(uint));
